...
template <typename... Args>
std::string test_std_sprintf(fmt::string_view format, const Args&... args) {
  std::array<char, 40> buf;
  const auto len = std::snprintf(
      buf.data(), buf.size(),
      std::string(std::begin(format), std::end(format)).c_str(), args...);
  EXPECT_LE(len, buf.size());
...
#define EXPECT_PRINTF(expected_output, format, arg)         \
  EXPECT_EQ(expected_output, test_sprintf(format, arg))     \
      << "format fmt: " << format;                          \
  EXPECT_EQ(expected_output, test_std_sprintf(format, arg)) \
      << "format std: " << format
...
TEST(PrintfTest, SpaceIgnoredIfPlusFlag) {
  EXPECT_PRINTF("+42", "%+ d", 42);
  EXPECT_PRINTF("-42", "%+ d", -42);
  EXPECT_PRINTF("+42", "% +d", 42);
  EXPECT_PRINTF("-42", "% +d", -42);
...
TEST(PrintfTest, MinusAndZeroFlag) {
...
TEST(PrintfTest, MinusAndSpaceFlag) { EXPECT_PRINTF(" 42", "%- d", 42); }
